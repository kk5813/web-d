      ...
      <div v-for="item in discussContent" :key="item.id" class="msgCard">
        ...
        <div class="btn">
          <el-link
            v-if="!readonly"
            type="success"
            @click="importOtherOpinion(item.content)"
            >引用</el-link
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    importOtherOpinion(text) {
      this.discussionOpinion += text;
    },
    inputBoxShow2() {
    ...
    getOpinions() {
      getDiscussDetails(this.cid).then((res) => {
        res.discussion.reverse();
        this.discussContent = res.discussion;
        this.discussContent.forEach((e) => {
          if (e.DoctorID == this.$store.state.user.userInfo.userId) {
            this.discussionOpinion = e.content;
          }
        });
    ...
    stopConsulation() {
      console.log(this.discussResult.conclusion);
