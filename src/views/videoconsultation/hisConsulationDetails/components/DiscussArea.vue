<template>
  <div class="discussArea">
    <!-- <div style="margin:30px" v-if="discussContent.length == 0">
      暂无人发表会诊意见
    </div>
    <div class="msgArea">
    ...
    >
      <div>
        ...
        <div>
          <Prescription
            :prescription="this.discussResult.prescription"
          ></Prescription>
        </div>
        <div>
          <span class="subTitle">后续治疗：</span>
...
export default {
  ...
  methods: {
    ...
    stopConsulation() {
      if (
        this.discussResult.conclusion != "" &&
        this.discussResult.treatOpinion != ""
      ) {
        this.$confirm("是否确认结束会诊?", "提示", {
          ...
          .then(() => {
            let consultationId = this.$store.state.videoConsultation
              .consultationId;

            stopGroupConsultation(consultationId, this.discussResult).then(
              (res) => {
                if (res) {
                  this.$message({
                    type: "success",
    ...
    postOpinion() {
      if (this.discussionOpinion != "") {
        let consultationId = this.$store.state.videoConsultation.consultationId;
        postDiscussOpinion(consultationId, this.discussionOpinion).then(
          (res) => {
            if (res) {
