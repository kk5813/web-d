  ...
  <div>
    <div class="pageContent">
      <el-collapse v-model="pages.collapse_activeNames">
        ...
        <el-collapse-item
          v-show="
            $store.state.videoConsultation.consultationState != '等待患者确认'
          "
          name="2"
        >
        ...
        <el-collapse-item v-if="detailShowFlag" ref="PidDetails" name="3">
          ...
          <div
            :key="$store.state.videoConsultation.currentPid"
            class="container"
          >
            <Reference
            ...
            <div
              v-if="
                $store.state.videoConsultation.isHolder &&
                $store.state.videoConsultation.consultationSource != 'huanzhe'
              "
              class="clearfix"
            >
              <el-button
...
export default {
  ...
  methods: {
    backPage() {
      localStorage.setItem(
        "pid",
        this.$store.state.videoConsultation.currentPid
      );
      this.$store.commit(
        "patientTreatment/currentPidChange",
        this.$store.state.videoConsultation.currentPid
      );
      this.$store.commit(
        "patientDiag/patientDiag_PidChange",
        this.$store.state.videoConsultation.currentPid
      );
  ...
  mounted() {
    ...
    let pid = this.$store.state.videoConsultation.currentPid;

    getPatientInfo(pid).then((res) => {
