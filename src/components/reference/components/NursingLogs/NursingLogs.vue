  ...
  <div>
    ...
    <div>
      <div v-if="pages.newLogFlag">
        <Jilubiao :pid="pid" @commit="newLogCommit($event)"></Jilubiao>
      </div>
    </div>
        ...
        >
          <el-table-column fixed label="时间" width="100px">
            <template slot-scope="scope">
              <span>
                <p>{{ scope.row.date.split(" ")[0] }}</p>
                <p>{{ scope.row.date.split(" ")[1] }}</p>
              </span>
            </template>
          </el-table-column>
          <el-table-column
...
<script>
import { getNursingLogs } from "@api/operationmanage/operationmanage.js";
import { postNursingLogs } from "@api/patientInfo/patientinfo.js";
...
export default {
  ...
  methods: {
    newLogCommit(data) {
      postNursingLogs(this.pid, data).then((res) => {
        this.pages.newLogFlag = false;
        getNursingLogs(this.pid).then((res) => {
  ...
  mounted() {
    ...
    window.onresize = () => {
      return (() => {
        ...
        if (document.getElementById("treatlogbox")) {
        }
        document.getElementById("treatlogbox").style.maxWidth =
          parseInt((boxwidht - 240) * 0.9) + "px";
      })();
